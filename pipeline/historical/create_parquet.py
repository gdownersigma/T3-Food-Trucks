...
OUTPUT_DIR = "data/input/transaction"


def clear_output_directory() -> None:
    """Remove existing parquet output directory to prevent duplicates."""
    if os.path.exists(OUTPUT_DIR):
        shutil.rmtree(OUTPUT_DIR)
        print(f"Cleared {OUTPUT_DIR}")


def prep_for_partition(data: Union[pd.DataFrame, str]) -> pd.DataFrame:
